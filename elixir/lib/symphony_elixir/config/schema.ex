defmodule SymphonyElixir.Config.Schema do
  ...
  defmodule ClaudeCode do
    ...
    def changeset(schema, attrs) do
      schema
      |> cast(attrs, [:command, :turn_timeout_ms, :read_timeout_ms], empty_values: [])
      |> validate_required([:command])
      |> validate_change(:command, fn :command, command ->
        if command != "" and String.trim(command) == "" do
          [command: "can't be blank"]
  ...
  embedded_schema do
    ...
    embeds_one(:agent_execution, AgentExecution, on_replace: :update, defaults_to_struct: true)
  ...
  defp changeset(attrs) do
    ...
    |> cast_embed(:agent_execution, with: &AgentExecution.changeset/2)
