...
- A Symphony deployment configured with:
  ...
  # path, bindle_bin, owner_id_path, claims_ledger_path all take their documented defaults
  ```
...
mise exec -- mix test test/symphony_elixir/bindle_adapter_test.exs \
  ...
  test/symphony_elixir/bindle_claims_ledger_test.exs \
...
4. **Expect**: within one poll cycle, Symphony's orchestrator logs dispatching the task; `bindle work
   claim <id> --owner <owner>` is invoked (observable via the Bindle repo's own claim state — the task now
   shows as claimed by Symphony's persisted owner id).
...
8. (Optional, crash-recovery proof) Kill the Symphony process between step 4 and step 6, leaving a
   dangling Bindle claim; restart Symphony against the same repo/config. **Expect**: startup-time
   reconciliation releases the stale claim (SC-006) before normal polling resumes — confirm via the Bindle
   repo's claim state.
