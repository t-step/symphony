...
claim/release/done` CLI (never a raw database write); and (3) a required, generic correction splitting
`Tracker.Issue`'s combined admission+routing predicate so a successfully-claimed item is never preempted
merely because its `dispatchable` fact naturally flips to `false` once claimed. All three land as additive
...
release/done) — Symphony never opens a write-capable SQLite connection to any Bindle-owned file. New
local Symphony-side state: a persisted owner-identity file and a small local claims ledger for
crash-recovery reconciliation (see data-model.md).
...
enforcement); an injectable `:bindle_cli_module` seam (mirroring `gitlab/adapter.ex`'s existing
`:gitlab_client_module` injection pattern, `elixir/lib/symphony_elixir/gitlab/adapter.ex:47-48`) so the
CLI boundary is testable without an installed `bindle` binary; orchestrator/agent-runner integration
tests for the admission-vs-continuation fix, covering every existing adapter's continuation behavior
(Linear reassignment stop, Asana completed-vs-section-name) explicitly, not just the new adapter's own.
...
calls scoped only to Bindle's supported CLI verbs (FR-016), and the admission-vs-continuation invariant
(FR-017) applied generically, not as a Bindle-specific branch.

**Scale/Scope**: One new adapter package (6 modules, mirroring `local/`'s 4-module shape plus a CLI wrapper
and a claims ledger), one new optional callback pair on `tracker.ex`, one split predicate on
`tracker/issue.ex`, targeted changes at 5 call sites across `orchestrator.ex`/`agent_runner.ex`, one new
...
- **V. Avoid Unnecessary Abstraction** — PASS. No new orchestration framework, lifecycle ontology, or
  generic plugin system is introduced. The claims ledger (data-model.md) is a small, local, crash-recovery-
  only JSON file, not a new subsystem. The owner-identity mechanism is the minimum new state needed to
  satisfy FR-010/FR-011, not a general identity system.
...
- **VI. Preserve Execution Boundaries** — PASS, and central to this plan: the Bindle adapter and the
  ...
  `acquire_issue/2`/`release_issue/2` are scoped exclusively to the claim/release CLI calls (FR-017); any
  lifecycle-mutation tool follows the existing `agent_tool_specs/0`/`execute_agent_tool/3` boundary, scoped
  to the session's bound issue exactly like `local_tracker_set_state` (FR-018).
...
- data-model.md's Owner Identity and Local Claims Ledger are both confirmed as the minimum new state
  needed — a single persisted string and a small JSON map keyed by issue id — not a new subsystem.
  **V still PASS.**
- contracts/bindle-tracker-adapter.md's exact `System.cmd/3` invocation shape (args, `cd:`, exit-code/
  stderr interpretation) was verified directly against Bindle's actual CLI argument parsing and exit-code
  convention, not assumed. **VI still PASS** — no raw database write path exists anywhere in the design.
...
elixir/
├── lib/symphony_elixir/
│   ├── tracker.ex                    # MODIFIED: +acquire_issue/2, +release_issue/2 (@optional_callbacks)
│   ├── tracker/issue.ex              # MODIFIED: split routable?/2 into dispatchable?/1 + routed?/2
│   ├── orchestrator.ex               # MODIFIED: call acquire_issue/2 before spawn; release_issue/2 at
│   │                                 #   existing release points; reconcile_issue_state/4 and
...
│   ├── config/schema.ex              # MODIFIED: +resolve_tracker_provider("bindle", ...)
...
│   └── bindle/                       # NEW package, mirrors local/'s shape
│       ├── adapter.ex                # @behaviour SymphonyElixir.Tracker; delegates reads to Projection,
│       │                             #   acquire/release to Cli, exposes agent_tool_specs via AgentTool
...
│       ├── owner.ex                  # Persisted, stable deployment-local owner-identity string
│       ├── claims_ledger.ex          # Small local JSON ledger for startup-time stale-claim reconciliation
│       └── agent_tool.ex             # Scoped lifecycle-mutation tool (FR-018), mirrors local/agent_tool.ex
├── mix.exs / mix.lock                # UNCHANGED (exqlite already present)
└── test/symphony_elixir/
    ├── bindle_adapter_test.exs           # NEW: projection→Issue mapping, schema-version failure, RO enforcement
    ├── bindle_projection_test.exs        # NEW: projection open/validate/query unit tests
    ├── bindle_cli_test.exs               # NEW: CLI arg/exit-code/stderr interpretation, cwd scoping
    ├── bindle_owner_test.exs             # NEW: owner-identity persistence
    ├── bindle_claims_ledger_test.exs     # NEW: local claims ledger read/write/clear
    ├── bindle_agent_tool_test.exs        # NEW: scoped lifecycle tool
    ├── bindle_orchestrator_integration_test.exs  # NEW: acquire-before-spawn, release-at-existing-points,
    │                                             #   startup-time stale-claim reconciliation
