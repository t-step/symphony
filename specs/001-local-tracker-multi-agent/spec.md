...
- What happens when a required dependency for a configured work-tracking source or coding-agent execution integration is missing or invalid — either detectable at startup, or only encountered when launching or running a specific work-item attempt (for example, the selected coding-agent's executable or tooling cannot be found at attempt launch)? Does Symphony validate what it can eagerly at startup using the same operator-visible error-handling pattern it uses today for missing/invalid tracker or Codex configuration, and route anything only detectable later, at the attempt level, into its normal attempt failure/retry/observability path (FR-008)? (Temporary unavailability of the work-tracking source itself during polling or reconciliation is a distinct case, handled by Symphony's existing tracker/source failure handling rather than as an individual attempt failure — see the following edge case. Previously established local durable state that is missing, corrupt, or unreadable is also distinct, addressed by FR-013.)
...
- **FR-008**: Symphony's handling of a missing or invalid work-tracking-source or coding-agent-execution-integration dependency MUST depend on when and where the failure is detected:
  1. A dependency failure detectable at startup MUST be validated as early as practical and surfaced using the same operator-visible startup/configuration failure behavior Symphony uses today for missing/invalid tracker or Codex configuration.
  2. A coding-agent execution dependency failure encountered while launching or running a specific work-item attempt (for example, the selected coding-agent's executable or tooling is unavailable at attempt launch) MUST enter Symphony's existing work-item attempt failure/retry/observability path, the same way an equivalent failure does today, rather than crashing the process or silently skipping the affected work.
  3. A work-tracking-source read or availability failure encountered during polling, reconciliation, or another source refresh MUST follow Symphony's existing tracker/source failure handling — skipping the affected poll tick or reconciliation pass and retrying on the next one while leaving already-running work-item attempts undisturbed — rather than being specified as an automatic individual work-item attempt failure merely because the source was temporarily unavailable.
  4. Previously established local durable state that becomes missing, corrupt, or otherwise unreadable is not a dependency failure in the sense of (1)-(3); it remains governed by FR-013 as a source-level operator-visible failure, distinct from both an ordinary transient source-read failure and an individual work-item attempt failure.
...
- **IV-005**: Existing dynamic workflow-configuration reload semantics remain unchanged for the settings Symphony already applies dynamically today (for example polling cadence, concurrency limits, active/terminal states, workspace paths/hooks, and prompt content) — these continue to be detected and applied to future dispatch, retry, and hook execution without requiring a restart. The selected work-tracking source and the selected coding-agent execution integration are structural deployment configuration, not dynamically-reloaded settings: a change to either selection, configured through WORKFLOW.md, takes effect only starting with the Symphony process/service's next restart, not through ordinary dynamic reload. This requirement does not change the existing dynamic-reload behavior of any other workflow setting.
